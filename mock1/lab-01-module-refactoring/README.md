...
> **Note ☆ — Task 5 经典操作步骤**
>
> 1. 创建 `infra/shared` 和 `infra/application` 两个根目录，并分别准备 `backend.tf`、`providers.tf`、`versions.tf`、`variables.tf`、`main.tf` 和 `outputs.tf`。
> 2. 将 shared 资源配置放入 `infra/shared`，将 identity 和 compute 放入 `infra/application`；两个根模块分别执行 `terraform init`。
> 3. 备份原始 state，然后复制 state 作为拆分工作的临时副本；不要直接删除或重新创建真实资源。
> 4. 在 shared state 中移除 application 资源，在 application state 中移除 shared 资源；用 `terraform state list` 检查所有权是否正确。
> 5. 在 shared 根模块执行 `terraform apply -refresh=false`，保存共享输出；application 通过 `terraform_remote_state` 读取这些输出。
> 6. 分别为两个根模块配置 S3 backend，并使用 `terraform init -migrate-state` 迁移各自的 state。
> 7. 分别执行 `terraform validate` 和 `terraform plan`；LocalStack 若因 S3 刷新产生误报，可用 `terraform plan -refresh=false` 辅助确认配置本身无变更。
> 8. 最后用 `terraform state list`、`terraform output` 和基线 JSON 核对资源归属、输出契约及所有资源标识符均保持不变。
